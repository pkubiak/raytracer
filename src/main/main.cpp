...
void addTree(Group* g, int trunkTessel, float trunkHeight, float trunkRadius, float crownHeight, float crownRadius, int crownTessel, int crownSteps) {
    ...
    Vector ex = Vector(1.0f, 0.0f, 0.0f);
    Vector ey = Vector(0.0f, 0.0f, 1.0f);
    Vector ez = Vector(0.0f, 1.0f, 0.0f);
    //trunk
    ...
    for (int i=0; i<crownSteps; ++i) {
        ...
        g->add(new Sphere(Point(0.0f, trunkHeight + crownHeight - crownStep*1.5, 0.0f), trunkRadius, nullptr, nullptr));
    }
